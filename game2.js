...
// ==========================
// Shared action helpers so both keyboard and touch call the same logic
function doPickup1() {
...
function doAttack1() {
  ...
  hitMonsters.clear();
  hitBoss = false;
  // Auto-aim: find nearest monster or boss, else use facing direction
...
// Joystick outputs: normalised -1..1 movement injected into gameLoop via mobileJoy
let mobileJoyX = 0,
  mobileJoyY = 0;
...
function setupMobileControls() {
  ...
  // Prevent default touch scroll/zoom on canvas
  ...
  canvas.addEventListener(
    ...
    (e) => {
      ...
      for (const t of e.changedTouches) {
        // Title screen taps — let click handler take it (synthesised)
        if (gameMode === null) return;
        if (t.clientX < VIEW_WIDTH * 0.5 && !joystickActive) {
          joystickActive = true;
      ...
      for (const t of e.changedTouches) {
        if (t.identifier === joystickTouchId) {
          const dx = t.clientX - joystickBaseX,
            dy = t.clientY - joystickBaseY;
          const dist = Math.sqrt(dx * dx + dy * dy);
          const capped = Math.min(dist, JOYSTICK_RADIUS);
          const angle = Math.atan2(dy, dx);
          joystickKnobX = joystickBaseX + Math.cos(angle) * capped;
          joystickKnobY = joystickBaseY + Math.sin(angle) * capped;
          mobileJoyX = Math.cos(angle) * (capped / JOYSTICK_RADIUS);
          mobileJoyY = Math.sin(angle) * (capped / JOYSTICK_RADIUS);
        }
      }
    },
    { passive: false },
  ...
  hud.appendChild(btnAttack);

  // Pickup button — above attack
  const btnPickup = document.createElement("div");
  btnPickup.id = "btnPickup";
  btnPickup.textContent = "☑";
  btnPickup.style.cssText = [
    "position:absolute",
    "bottom:130px",
    "right:30px",
    "width:60px",
  ...
  hud.appendChild(btnPickup);

  // Wire up button events (touchstart for instant response)
  const onTouchBtn = (el, fn) => {
    el.addEventListener(
      "touchstart",
      (e) => {
        e.preventDefault();
    ...
    // Also support mouse clicks for desktop testing of mobile layout
  ...
  onTouchBtn(btnAttack, () => {
    if (gameMode !== null && !player1.dead) doAttack1();
  });
  onTouchBtn(btnPickup, () => {
    if (gameMode !== null && !player1.dead) doPickup1();
  });

  // Show/hide pickup button based on whether there's a nearby item
  // (updated each frame in drawMobileHud)
}

function drawMobileHud() {
  if (!isMobile || gameMode === null) return;

  // Update pickup button visibility
  const btnPickup = document.getElementById("btnPickup");
  if (btnPickup) {
    btnPickup.style.opacity = nearbyItem ? "1" : "0.35";
    btnPickup.style.transform = nearbyItem ? "scale(1.1)" : "scale(1)";
  }

  // Update attack button glow when weapon equipped
  const btnAttack = document.getElementById("btnAttack");
  if (btnAttack) {
    btnAttack.style.background =
      player1.item && !swinging ? `rgba(220,60,60,0.9)` : `rgba(120,40,40,0.7)`;
    btnAttack.style.boxShadow = player1.item
      ? `0 4px 16px rgba(255,100,100,0.7)`
      : `0 4px 8px rgba(0,0,0,0.4)`;
  }

  // Draw the virtual joystick on the canvas
  if (joystickActive) {
    const ctx = ctx2d;
    // Outer ring
    ctx.save();
    ...
    // Draw a faint ghost joystick hint bottom-left
    const gx = 80,
      gy = VIEW_HEIGHT - 90;
    ctx2d.save();
    ctx2d.globalAlpha = 0.18;
    ctx2d.beginPath();
    ctx2d.arc(gx, gy, JOYSTICK_RADIUS, 0, Math.PI * 2);
    ctx2d.fillStyle = "#fff";
    ctx2d.fill();
    ctx2d.beginPath();
    ctx2d.arc(gx, gy, JOYSTICK_KNOB_RADIUS, 0, Math.PI * 2);
    ctx2d.fillStyle = "#aaa";
    ctx2d.fill();
    ctx2d.restore();
  }
